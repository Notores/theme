...
const getFileName = async (themeTypeConfig, themePath, path, page) => {
	...
	if (themeTypeConfig.isApp) {
		...
		if (page) {
            ...
            checkFilenames.push(`../private/pages/${page}.ejs`);
        ...
        if(path.toString().indexOf('n-admin') > -1)
            path = path.toString().replace('n-admin/','');


        checkFilenames.push(path);
		checkFilenames.push(`${path}.ejs`);
		checkFilenames.push(`/pages${path}.ejs`);
        checkFilenames.push(`../private/pages${path}.ejs`);
	...
	return page === 404 ? 'ERROR' : getFileName(themeTypeConfig, themePath, path, 404);
};
